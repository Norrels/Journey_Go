...
func run(ctx context.Context) error {
	...
	pool, err := pgxpool.New(ctx, fmt.Sprintf(
		...
		os.Getenv("JOURNEY_DATABASE_PASS"),
	...
	fmt.Println("Conectado com o banco!")

	si := api.NewAPI(pool, logger)
	route := chi.NewMux()
	route.Use(middleware.RequestID, middleware.Recoverer, httputils.ChiLogger(logger))
	route.Mount("/", spec.Handler(&si))
	...
	defer func() {
		...
		if err := srv.Shutdown(ctx); err != nil {
			logger.Error("failed to shoutdown server", zap.Error(err))
		}
	}()
